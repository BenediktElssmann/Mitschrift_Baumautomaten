...
Wir vereinbaren Abk\"urzungen: NBA/NTA nichtdeterministischer Baumautomat und DBA/DTA deterministischer Baumautomat.\\
...
$\mathcal{A} = (Q, \Sigma, F, \Delta)$ mit $Q = \{q, q_1, \dots , q_{n+1} \}$, $F = \{q_{n+1}\}$ mit \"Uberg\"angen
...
\section{Definition vollst\"anding und reduziert}
...
zerlegen l\"asst in $w = xyz$, $y \neq \varepsilon$ und $\forall i \geqq 0$ ist $xy^iz \in L$. \\ \\
...
\section{Definition Kontext}

Es sei $\Sigma$ ein Fangalphabet und $x^{(0)} \notin \Sigma$. Es sei $C \in T_{\Sigma \cup \{x\}}$.
