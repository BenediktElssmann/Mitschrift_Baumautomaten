...
\begin{tikzpicture}[shorten >= 1pt, node distance=2cm, on grid, auto]
    ...
    \path[->]   (q1)    edge    [loop above]    node    {a,b}   ();
\end{tikzpicture}
...
\begin{tikzpicture}[shorten >= 1pt, node distance=2cm, on grid, auto]
    ...
    \path[->]   (q0)    edge                    node    {a,1} (q1);
    \path[->]   (q0)    edge    [loop above]    node    {a,1;b,1}   ();
    \path[->]   (q1)    edge    [loop above]    node    {a,1;b,1}   ();
\end{tikzpicture}
...
Betrachte\\
\begin{tikzpicture}[shorten >= 1pt, node distance=2cm, on grid, auto]
    \node[state, initial, initial text=1] (q1) {$ q_1 $};
    \node[state] (q3) [below=of q1 ] {$ q_3 $};
    \node[state, initial, initial text=2] (q2) [below=of q3 ] {$ q_2 $};
    \node[state, accepting] (q4) [right=of q3 ] {$ q_4 $};

    \path[->]   (q1)    edge                    node    {$a|2$} (q3);
    \path[->]   (q1)    edge                    node    {$a|5$} (q4);
    \path[->]   (q1)    edge    [loop above]    node    {$a|2$} ();
    \path[->]   (q2)    edge                    node    {$a|1$} (q3);
    \path[->]   (q2)    edge                    node    {$a|4$} (q4);
    \path[->]   (q2)    edge    [loop below]    node    {$a|2$} ();
    \path[->]   (q3)    edge                    node    {$a|3$} (q4);
\end{tikzpicture}
...
\begin{tikzpicture}[shorten >= 1pt, node distance=2cm, on grid, auto]
    \node[state] (q1) {$ 2 $};
    \node[state] (q3) [below=of q1 ] {$ 4 $};
    \node[state] (q2) [below=of q3 ] {$ 4 $};
    \node[state, accepting] (q4) [right=of q3 ] {$ 13 $};

    \path[dashed]   (q1)    edge    node    {$a|2$} (q3);
    \path[dashed]   (q2)    edge    node    {$a|1$} (q3);
    \path[dashed]   (q3)    edge    node    {$a|3$} (q4);
\end{tikzpicture}
